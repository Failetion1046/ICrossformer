...
class Dataset_ETT_hour(Dataset):
    def __init__(self, root_path, flag='train', size=None,
        ...
        self.__read_data__()
        if gety==0:
            self.y_pred = ClusteringPred(self.data_x,numcluster)
        else:
            self.y_pred=None
...
def ClusteringPred(x,num_cluster):

    X = x[:512,:].transpose(-1, -2)
    ...
    # plt.tight_layout()
    # plt.show()
    # plt.savefig('./pic-{}.png'.format(X[1,1]))
